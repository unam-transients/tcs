...
# $Id: check_openmanage.cgi 3558 2020-05-22 18:41:08Z Alan $
...
exec tclsh8.6 -encoding "utf-8" "$0" ${1+"$@"}
  ...
  set text "$text\$ /usr/lib/nagios/plugins/check_openmanage -d --no-storage\n"
  set channel [open "|/bin/sh check_openmanage.sh" "r"]
  ...
  puts "<html>"
  puts "<head>"
  puts "<title>TCS: check_openmanage</title>"
  puts "<link rel=\"stylesheet\" href=\"style.css\" type=\"text/css\"/>"
  puts "</head>"
  puts "<body>"
  puts "<h1>TCS: check_openmanage</h1>"
  puts "<pre>$host</pre>"
