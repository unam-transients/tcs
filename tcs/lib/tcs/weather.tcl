...
########################################################################
#
# Copyright © 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2017, 2019, 2026
# Alan M. Watson <alan@astro.unam.mx>
#
...
namespace eval "weather" {
  ...
  proc parsedata {datalines} {
    ...
    foreach dataline $datalines {
      log::debug "parsing data line."
      log::debug [format "data line is \"%s\"." $dataline]
      if {
        ...
        || [scan $dataline "b.1 %s %s a %*s %*s %s %*s %*s %s %s %f %f %f %*f %*f %f %*f %*f %*f %*f %f %*f %*f %*f %f %*f %*d %d %d %*d %*d %*d %d" pendingdate pendingtime pendingwindaverageazimuth pendingwindaveragespeed pendingwindgustspeed pendingtemperature pendinghumidity pendingpressure pendingrainrate pendingskytemperature pendingdewpoint pendingrainindex pendingcloudindex pendinglightindex] == 14
      } {
        log::debug [format "data line is from the COLIBRÍ PLC."]
        ...
        [scan $dataline "%s %s %f %*f %*f %f %f %f %f %f %f %f" pendingdate pendingtime pendingtemperature pendinghumidity pendingdewpoint pendingwindaveragespeed pendingwindgustspeed pendingwindaverageazimuth pendingrainrate pendingpressure] == 10
      } {
    ...
    if {[string equal [server::getdata "windaverageazimuth"] "unknown"]} {
      log::writesensorsfile "weather-wind-average-azimuth" [server::getdata "windaverageazimuth"] \
        [server::getdata "timestamp"]
    } else {
      log::writesensorsfile "weather-wind-average-azimuth" \
        [astrometry::radtodeg [server::getdata "windaverageazimuth"]] [server::getdata "timestamp"]
    }
  }
  ...
  proc getdatafiles {} {
    return [lrange [lsort -increasing [glob -nocomplain -directory "[directories::var]/weather" "*.txt"]] end-2 end]
  }
