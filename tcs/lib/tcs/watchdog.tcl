...
namespace eval "watchdog" {
  ...
  proc notify {} {
    ...
    log::warning \
      [format "persistent problems since %s with: %s" [utcclock::format $problemtimestamp 0] \
      [join $problemservers " "] ]
    ...
    set message \
      [format "watchdog: persistent problems since %s with: %s" [utcclock::format $problemtimestamp 0] \
      [join $problemservers " "] ]

    log::info "sending emergency pushover message \"$message\"."
    if {[catch {
      exec "[directories::prefix]/bin/tcs" "sendpushover" "-P" "emergency" "-s" "Watchdog" "emergency" "$message"
    }]} {
  ...
  proc monitorservers {} {
    if {[catch {
      ...
      if {[llength $problemservers] == 0} {
        ...
        log::info [format "problems since %s with: %s" [utcclock::format $problemtimestamp 0] [join $problemservers " "] ]
      }
      ...
      log::warning "while monitoring servers: $message"
    }
  }
