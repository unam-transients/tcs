...
proc alertvisit {{filter "w"}} {
    ...
    log::summary \
        [format "alertvisit: alert coordinates are %s %s %s." [astrometry::formatalpha $alpha]  \
    ...
    if {[string equal "" [alert::eventtimestamp [executor::alert]]]} {
        ...
        log::summary \
            [format "alertvisit: event timestamp is %s." [utcclock::format [alert::eventtimestamp [executor::alert]]]]
    }
    if {[string equal "" [alert::alerttimestamp [executor::alert]]]} {
        log::summary [format "alertvisit: no alert timestamp."]
    } else {
        log::summary \
            [format "alertvisit: alert timestamp is %s." [utcclock::format [alert::alerttimestamp [executor::alert]]]]
    }
    ...
    log::summary \
        [format "alertvisit: %d fields with %d dithers per field and %d exposures per dither." $fields \
        $dithersperfield $exposuresperdither]
    log::summary \
        [format "alertvisit: total of %d exposures of %.0f seconds with binning of 1." \
    ...
    while {$dither < $dithersperfield} {
        ...
        log::info \
            [format "alertvisit: dither %d is %+.2fd east and %+.2fd north." $dither \
            [astrometry::radtodeg $dithereastoffset ] [astrometry::radtodeg $dithernorthoffset] ]
        ...
        foreach {visitidentifier visiteastoffset visitnorthoffset} $visits {
            ...
            while {$exposure < $exposuresperdither} {
                if {$first} {
                    set alertdelay [alert::delay [executor::alert]]
                    log::summary \
                        [format "alertvisit: alert delay at start of first exposure is %.1f seconds (%.1f hours)." \
                        $alertdelay [expr {$alertdelay / 3600}]]
                    log::summary \
    ...
    log::summary \
        ...
        [expr {$alertdelay / 3600}]]
    log::summary \
        [format "alertvisit: alert coordinates after end of last exposure are %s %s %s." \
        [astrometry::formatalpha $alpha]  [astrometry::formatdelta $delta] $equinox]
...
proc starevisit {exposures exposuretime {filters "w"}} {
    ...
    log::summary [format "starevisit: %d × %.0f second exposures." $exposures $exposuretime ]
...
proc gridvisit {gridrepeats gridpoints exposuresperdither exposuretime {filters "w"}} {
    ...
    log::summary \
        [format "gridvisit: %d × %.0f second exposures." [expr {$gridrepeats * $gridpoints * $exposuresperdither}] \
        $exposuretime ]
...
proc steppedgridvisit {gridrepeats exposuresperdither exposuretime} {
    ...
    log::summary \
        [format "steppedgridvisit: %d × %.0f second exposures." [expr {$gridrepeats * 5 * $exposuresperdither}] \
        $exposuretime ]
...
proc allskyvisit {} {
    ...
    log::summary \
        [format "allskyvisit: %d × %.0f second exposures." [expr {$gridrepeats * $gridpoints * $exposuresperdither}] \
        $exposuretime ]
...
proc focusmapvisit {} {
    ...
    log::summary \
        [format "focusmapvisit: focusing at %s %s." [astrometry::formatha $ha]  [astrometry::formatdelta $delta]]
    ...
    if {[string equal $worstfwhmarcsec "unknown"] || $worstfwhmarcsec > 100} {
        log::summary "focusmapvisit: aborting with worst witness FWHM of $worstfwhmarcsecas."
    ...
    log::summary "focusmapvisit: succeeded with worst witness FWHM is $worstfwhmarcsecas."
    log::summary "focusmapvisit: finished."
...
proc twilightflatsvisit {} {
    ...
    if {$ngood == 0} {
        ...
        log::summary \
            [format "twilightflatsvisit: $ngood good flats (%.0f to %.0f DN) with filter $filter." $mingoodlevel \
            $maxgoodlevel]
    }
    if {$evening} {
