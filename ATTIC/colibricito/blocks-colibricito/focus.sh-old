...
rm -f 2019A-0004-initial-focus-*
rm -f 2019A-0004-focus-*
rm -f 2019A-0007-donut-*

focusfilter="w"
correctpointingfilter="w"

while read suffix targetid tyc alpha delta TYC ALPHA DELTA
do
  cat <<EOF >2019A-0004-initial-focus-$suffix
proposal::setidentifier "2019A-0004"
block::setidentifier $targetid
visit::setidentifier 0
visit::setname "initial pointing correction and focus check on TYC-$tyc"
...
proc EXECUTE {args} {
  ...
  analyze fwhm
  client::update C0
  set fwhm [client::getdata "C0" "fwhm"]
  if {[string equal \$fwhm ""]} {
    set fwhm "unknown"
  } else {
    set fwhm [format "%.2f" \$fwhm]
  }
  log::summary "witness FWHM is \$fwhm pixels with binning 1 in filter w in 60s in C0."
  setfocused
...
EOF
  ...
  cat <<EOF >2019A-0004-focus-swift-$suffix
proposal::setidentifier "2019A-0004"
block::setidentifier $targetid
visit::setidentifier 1
visit::setname "focus check on TYC-$tyc"
block::settotalexposures 0
visit::settargetcoordinates equatorial $alpha $delta 2000
...
EOF
  cat <<EOF >2019A-0004-focus-not-swift-$suffix
proposal::setidentifier "2019A-0004"
block::setidentifier $targetid
visit::setidentifier 1
visit::setname "focus check on TYC-$tyc"
